...
try:
    lib_path=os.path.join( os.environ["CPYKDTREELIB"],"libkdtree.so")
except KeyError:
    lib_path="/usr/local/lib/libkdtree.so"
_lib = C.CDLL(lib_path)


# You can pick wether you want to use floats or double in the
# c code. To changes from one to another, change the float_choice
# variable, then use the search and replace script "switch_precision"
# to go float, place yourself in the cPykdtree directory and:
#      ./switch_precision double float
# And to go double:
#      ./switch_precision float double
# Remember to "make" the libraries

# Double is standard, but if you're working on large systems, 
# simple float is probably a better choice, memory wise.

#float_choice = "float"
float_choice = "double"

if float_choice == "float":
    FLOAT = C.c_float
    numpy_float = np.float
if float_choice == "double":
    FLOAT = C.c_double
    numpy_float = np.double

FLOAT_pointer=C.POINTER(FLOAT)
